...
export default function LoginPage() {
  ...
  async function handleGoogleLogin() {
    ...
    // redirectTo WAJIB persis sama (origin + path) dengan salah satu URL yang
    // didaftarkan di Supabase Dashboard > Authentication > URL Configuration
    // > Redirect URLs. window.location.origin otomatis jadi localhost:3000
    // saat dev dan domain vercel.app saat production, jadi satu baris kode
    // ini otomatis benar di kedua environment asal keduanya sudah terdaftar
    // di Supabase (sudah, per screenshot konfigurasi kamu).
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: `${window.location.origin}/auth/callback` },
    });
    if (error) {
      setError("Gagal memulai login Google: " + error.message);
      setGoogleLoading(false);
    }
    // Kalau sukses, browser langsung di-redirect ke Google -- tidak perlu
    // setGoogleLoading(false) di sini karena halaman ini akan segera
    // ditinggalkan.
  }
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="rounded-2xl bg-white p-8 shadow-sm ring-1 ring-slate-200">
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-md border border-slate-200 bg-white py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 disabled:opacity-60"
            >
